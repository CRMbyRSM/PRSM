        ...
        <section className="detail-section">
          ...
          <form className="create-agent-form" onSubmit={handleSubmit}>
            <div className="create-agent-avatar-section">
              <div className="create-agent-avatar-picker" onClick={() => fileInputRef.current?.click()} title="Click to upload avatar">
                {avatarPreview ? (
              ...
              {avatarPreview && (
                <button type="button" className="create-agent-avatar-remove" onClick={handleRemoveAvatar}>Remove</button>
              )}
            ...
            <div className="form-group">
              ...
              <input id="agent-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="My Agent" autoFocus disabled={creating} />
            </div>

            <div className="form-group">
              <label htmlFor="agent-workspace">Workspace Path <span className="required">*</span></label>
              <input id="agent-workspace" type="text" value={workspace} onChange={(e) => setWorkspace(e.target.value)} placeholder="~/agents/my-agent" disabled={creating} />
              <span className="form-hint">Absolute path on the server where agent files will be stored</span>
            </div>

            <div className="form-group">
              <label htmlFor="agent-model">Model</label>
              <input id="agent-model" type="text" value={model} onChange={(e) => setModel(e.target.value)} placeholder="e.g. claude-sonnet-4-5-20250929" disabled={creating} />
              <span className="form-hint">LLM model identifier (optional, can be set later)</span>
            </div>

            <div className="form-group">
              <label htmlFor="agent-emoji">Emoji</label>
              <input id="agent-emoji" type="text" value={emoji} onChange={(e) => setEmoji(e.target.value)} placeholder="e.g. 🤖" maxLength={4} disabled={creating} className="emoji-input" />
            </div>
            ...
            <div className="create-agent-actions">
              <button type="button" className="btn btn-secondary" onClick={closeDetailView} disabled={creating}>Cancel</button>
